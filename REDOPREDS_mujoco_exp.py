...
parser.add_argument("--redopreds", type=toBool, default=False)
...
class Trajectory:
    ...
    def retrieveStateFeatures(self, idx, context = args.context):
        mindx = max(idx - context, 0)
...
class Buffer:
    ...
    def sample(self, size):
        ...
        idxs = np.sort(idxs)
        if args.statepred and args.redopreds:
            statepredfeats = self.getStatePredFeatures(idxs, context = 5)[0]
            idxsprev = np.where(np.array(idxs) > 0, idxs - 1, 0)
            statepredprev = self.getStatePredFeatures(idxsprev, context = 5)[0]
        else:
            statepredfeats = None
            statepredprev = None
        i = 0
        ...
        return toReturn, statepredfeats, statepredprev
    ...
    def sampleSubSeqs(self, subLen, numSubs):
        ...
        # print(idxs, "!!")
        for i in idxs:
            idxs2 = np.random.choice(self.els[i].length, subLen, replace = subLen>self.els[i].length)
            temp = self.els[i].sample(idxs2)
            if args.statepred and args.redopreds:
                tsp = self.getStatePredFeatures(idxs2, context = 5)
                temp['statefeats'] = tsp[0]
                lengths = np.array(tsp[3])
                idxsprev = np.where(lengths > 1, idxs2 - 1, 0)
                temp['statefeatsprev'] = self.getStatePredFeatures(idxsprev, context = 5)[0]
    ...
    def getStatePredFeatures(self, idxs, context = args.context):
        idxs = sorted(idxs)
        idxs = np.maximum(idxs, np.zeros_like(idxs))
        i = 0
        ...
        for idx in idxs:
            ...
            if (idx - offset) < 0:
                print("ERRORR!!!!!!! Buffer index offset wrong")
            tfeat, tlab, tknown = self.els[i].retrieveStateFeatures(idx - offset, context)
    ...
    def sampleForStatePred(self, size):
        idxs = np.random.choice(self.count, size, replace = size>self.count)
        return self.getStatePredFeatures(idxs)
...
for step in range(int(args.numSteps)):
    ...
    if (numSteps) >= args.startLearning:
        for trainstep in range(args.train_batches):
            ...
            knownprevSamp = np.array(samples['knowns'])
            knownSamp = np.array(samples['nextknowns'])
            
            if args.statepred and args.redopreds:
                with torch.no_grad():
                    temppreds = []
                    for ft in samples['statefeatsprev']:
                        if args.cuda:
                            ft = ft.cuda()
                        temppred = statepred_target(ft)
                        if args.cuda:
                            temppred = temppred.cpu()
                        temppred = temppred.numpy()
                        temppreds.append(temppred)
                    obSamp = np.array(temppreds) * (1 - knownprevSamp) + np.array(samples['obs']) * knownprevSamp
                    temppreds = []
                    for ft in samples['statefeats']:
                        if args.cuda:
                            ft = ft.cuda()
                        temppred = statepred_target(ft)
                        if args.cuda:
                        ...
                        temppred = temppred.numpy()
                        temppreds.append(temppred)
                    ob2Samp = np.array(temppreds) * (1 - knownSamp) + np.array(samples['nextobs']) * knownSamp
            else:
            ...
            samples, statepredfeats, statepredprev = buff.sample(256)
            ...
            knownSamp = np.array([samp['nextknowns'] for samp in samples])
            knownprevSamp = np.array([samp['knowns'] for samp in samples])
            donesSamp = torch.tensor([samp['dones'] for samp in samples]).float().unsqueeze(-1)
            if args.statepred and args.redopreds:
                with torch.no_grad():
                    if args.cuda:
                        ft = statepredfeats.cuda()
                    temppred = statepred_target(ft)
                    if args.cuda:
                        temppred = temppred.cpu()
                    temppred = temppred.numpy()
                    nextObsSamp = np.array(temppred) * (1 - knownSamp) + np.array([samp['nextobs'] for samp in samples]) * knownSamp

                    if args.cuda:
                        ft = statepredprev.cuda()
                    temppred = statepred_target(ft)
                    if args.cuda:
                        temppred = temppred.cpu()
                    temppred = temppred.numpy()
                    obSamp = np.array(temppred) * (1 - knownprevSamp) + np.array([samp['obs'] for samp in samples]) * knownprevSamp
            else:
    ...
    if numSteps >= args.startLearningState:
        if (step % 50) == 0 or (step == args.numSteps - 1):
            ...
            if args.contextSAC:
                sacstr = f"{args.context}cSAC"
            foldern = f"./saved_mujoco/{args.env}/{sacstr}/{statemodel}/"
            fname = f"{args.numHidden}Hidden{args.seed}QLR{args.qlr}ALR{args.actlr}SLR{args.statelr}Start{args.startLearningState},{args.startLearning}HS{args.hiddenSizeLSTM}C{args.context}REDO{args.redopreds}.csv"
            with torch.no_grad():
