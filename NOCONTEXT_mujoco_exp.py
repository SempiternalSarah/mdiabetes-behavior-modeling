...
parser.add_argument("--numSteps", type=int, default=30000, help="iterations of overall algorithm")
...
parser.add_argument("--consecHidden", type=int, default=5, help="no. consecutive timesteps to hide information (e.g. hide an ant limb for 10 environmental steps)")
...
class SoftQNetwork(nn.Module):
    def __init__(self, envs):
        ...
        self.fc1 = layer_init(nn.Linear(self.input_shape, 256))
        self.fc2 = layer_init(nn.Linear(256, 256))
        self.fc_q = layer_init(nn.Linear(256, 1))
...
class Actor(nn.Module):
    def __init__(self, envs: gym.Env):
        ...
        self.fc1 = layer_init(nn.Linear(self.obs_shape[0], 256))
        self.fc2 = layer_init(nn.Linear(256, 256))
        self.fc_mean_logdev = layer_init(nn.Linear(256, 2*self.action_shape[0]))
...
def getStateBelief(observations, knowns, acts=None, statepred = None):
    if args.statepred and args.numHidden > 0:
...
if args.statepred:
    ...
    statestr += '/'
if (args.logging):
    if not os.path.exists(f"./saved_mujoco/{args.env}/{statestr}rewards"):
        os.makedirs(f"./saved_mujoco/{args.env}/{statestr}rewards")
    if not os.path.exists(f"./saved_mujoco/{args.env}/{statestr}actloss"):
        os.makedirs(f"./saved_mujoco/{args.env}/{statestr}actloss")
    if not os.path.exists(f"./saved_mujoco/{args.env}/{statestr}qfloss"):
        os.makedirs(f"./saved_mujoco/{args.env}/{statestr}qfloss")
    if not os.path.exists(f"./saved_mujoco/{args.env}/{statestr}rrdloss"):
        os.makedirs(f"./saved_mujoco/{args.env}/{statestr}rrdloss")
    if not os.path.exists(f"./saved_mujoco/{args.env}/{statestr}statepredloss"):
        os.makedirs(f"./saved_mujoco/{args.env}/{statestr}statepredloss")
    if not os.path.exists(f"./saved_mujoco/{args.env}/{statestr}statepredtestloss"):
        os.makedirs(f"./saved_mujoco/{args.env}/{statestr}statepredtestloss")
        ...
        if (step % 50) == 0 or (step == args.numSteps - 1):
            ...
            if (args.logging):
                if args.statepred:
                    ...
                    statemodel = ""
                fname = f"{args.numHidden}Hidden{args.consecHidden}HTime{args.seed}LR{args.qlr}.csv"
                np.savetxt(f"./saved_mujoco/{args.env}/{statemodel}rewards/{fname}", rewardList, delimiter="\n")
                if not actloss is None:
                    ...
                    if (args.statepred):
                        ...
                        np.savetxt(f"./saved_mujoco/{args.env}/{statemodel}statepredtestloss/{fname}", testStateLossList, delimiter="\n")
                        np.savetxt(f"./saved_mujoco/{args.env}/{statemodel}statepredloss/{fname}", statelosslist, delimiter="\n")

                    np.savetxt(f"./saved_mujoco/{args.env}/{statemodel}actloss/{fname}", actlosslist, delimiter="\n")
                    np.savetxt(f"./saved_mujoco/{args.env}/{statemodel}qfloss/{fname}", qflosslist, delimiter="\n")
                    np.savetxt(f"./saved_mujoco/{args.env}/{statemodel}rrdloss/{fname}", rrdlosslist, delimiter="\n")
