...
envs = ["Ant-v2", "Humanoid-v2", "HalfCheetah-v2"]
...
for maxEpoch in range(250, 700, 25):
    for env in envs:
        for i, hidden in enumerate(hiddens):
                            ...
                            for s, statepred in enumerate(statepreds):
                                for sacount, htime in enumerate(htimes):
                                    ...
                                    for seed in range(seedmin, seedmax+1):
                                        ...
                                        if statepred == 'lstm':
                                            fname = f"{folder}/{hidden}Hidden{seed}QLR{qlr}ALR{alr}SLR{slr}Start{start},{start2}HS{hs}C{context}RST-1.csv"
                                        else:
                                            fname = f"{folder}/{hidden}Hidden{seed}QLR{qlr}ALR{alr}SLR{slr}Start{start},{start}HS256C{context}RST-1.csv"
            ...
            if 'qfloss' in variables:
                ...
                plt.ylabel("Loss sqrt (MSE)", fontsize=20)
                name = "ASTATEPRED"
